...
export class AddictComponent implements OnInit {
  ...
  public loadData() {
    ...
    if (window.localStorage['isAdmin']) {
      this.service.getAll().subscribe({
        next: data => {
          this.dataSource = new DataSource({ store: data, reshapeOnPush: true });
          this.rountCount = data.length;
        },
        error: () => console.log("loi gi o day")
      });
      //   next:
      //   data => {
      //   //console.log(data);
  ...
  //   console.log(this.managePlaceData);
  // }, err => console.log(err));
  // }
